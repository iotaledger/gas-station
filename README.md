...
secret token. An internal server that communicates with the Gas Station must specify the token in the request. This
...
The Gas Stations exposes Prometheus metrics endpoints with a variety of metrics. The port can be configured in `config.yaml` using the `metrics-port` setting.
...
| `rpc-port`              | The port that the RPC server runs on.                                                                                                                                                                       |
...
| `coin-init-config`      | Configuration for coin initialization.                                                                                                                                                                      |
| `target-init-balance`   | The targeting initial balance of each coin (in MIST). For instance, if you specify `100000000` which is `0.1 IOTA`, the Gas Station will attempt to split its gas coin into smaller gas coins each with `0.1 IOTA` balance during initialization. |
| `refresh-interval-sec`  | The interval to look at all gas coins owned by the sponsor again and see if some new funding has been added.                                                                                                |
